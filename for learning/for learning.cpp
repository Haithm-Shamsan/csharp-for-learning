// for learning.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
#include<iomanip>
using namespace std;
enum enMainScreenOptions { ClientList = 1, AddClinet = 2, DeleteClinet = 3, UpdateClinet = 4, FindClient = 5, Transaction = 6, MangeUsers = 7,LogOut=8 };
enum enMangeUserOptions { UserLest=1,AddUser=2,DeleteUser=3,Updateuser4,FindUser=5,enBack=6
};
...
string ConvertUserRecordToLine(sUser User, string Seperator = "#//#") {
    ...
    stUserRecord += to_string(User.UserPermations) + Seperator;
    return stUserRecord;
}
...
sUser ConvertUserLinetoRecord(string Line, string Seperator = "#//#") {
    ...
    User.UserPermations = stoi(vUserData[2]);
...
vector<sUser>LoadUserDataFormFile(string FileName)
{
...
void PrintUserRecord(sUser User) {
    ...
    cout << "| " << setw(40) << left << User.UserPermations;
...
void ShowAllUsersInTable()
{
    vector<sUser>vUsers = LoadUserDataFormFile(UsersFileName);
...
void ShowFindUserByUsernameAndPassword()
{
;
    ...
    if (FindClientByAccountNumber(User,Password,CurrntUser))
    {
        return ture
...
//                      Transaction 
enum enTransaction {
    enDeposit = 1, enWithDrows = 2, enTotalBalances = 3, BackToMainMenue = 4
};
//                               Add Balance 
...
void PreformMangeUserScreen(enMangeUserOptions User)
{
    switch (User)
    {
    case UserLest:
...
void ShowMangeUserMenue()
{
    ...
    cout << "===========================================\n";
    PreformMangeUserScreen(enMangeUserOptions(ReadScreenChoise()));
}
